...
select avg(cast(display_size as numeric))
from smartphone;
...
join os o on s.id_os = o.id
join marca m on s.id_marca = m.id
where (m.brand) in ('Samsung', 'Apple');
...
select s.nome, s.peso
from smartphone s, os o
where s.id_os = o.id and o.descrizione = 'Android' and s.peso between 130 and 150
and s.peso >= all(
    select s.peso
    from smartphone s, os o
    where s.id_os = o.id and o.descrizione = 'Android'
    and s.peso between 130 and 150
);